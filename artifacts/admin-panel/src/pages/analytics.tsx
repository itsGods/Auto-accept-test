              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border">
                    ...
                    <th className="text-right py-2 text-green-400 font-medium">Approved</th>
                    <th className="text-right py-2 text-red-400 font-medium">Rejected</th>
                    <th className="text-right py-2 text-primary font-medium">New Users</th>
                  </tr>
                </thead>
                <tbody>
                  {daily?.map((d) => (
                    <tr key={d.date} className="border-b border-border/30 hover:bg-muted/20">
                      <td className="py-2 text-muted-foreground">{new Date(d.date).toLocaleDateString("en", { weekday: "short", month: "short", day: "numeric" })}</td>
                      <td className="py-2 text-right">{d.totalRequests}</td>
                      <td className="py-2 text-right text-green-400">{d.approved}</td>
                      <td className="py-2 text-right text-red-400">{d.rejected}</td>
                      <td className="py-2 text-right text-primary">{d.newUsers}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
